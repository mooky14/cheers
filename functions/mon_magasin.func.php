...
// Je récupère les données de la BDD et je créé mes variables : 
    ...
    while ($donnees = $informations_magasins->fetch()){
        $get_id_magasin = $donnees['id_magasin'];
...
if($_GET['action'] == "new") {
    ...
    // !empty($_POST['usershop']) && !empty($_POST['password']) && !empty($_POST['password-confirm']) && !empty($_POST['datenaiss'])
       ...
       if(sizeof($messages_erreurs2) != 0) { // Erreurs détectées, on les affiche
        ...
        if(empty($_POST['usershop'])) {$messages_erreurs2[sizeof($messages_erreurs2)]=array("field"=>"usershop", "message"=>"Nom d'utilisateur non renseigné.");}
        if(empty($_POST['datenaiss'])) {$messages_erreurs2[sizeof($messages_erreurs2)]=array("field"=>"datenaiss", "message"=>"Date de naissance non renseigné.");}
        if(empty($_POST['ville_magasin'])) {$messages_erreurs2[sizeof($messages_erreurs2)]=array("field"=>"ville_magasin", "message"=>"ville_magasin non renseignée.");}
        if(empty($_POST['postal_magasin'])) {$messages_erreurs2[sizeof($messages_erreurs2)]=array("field"=>"postal_magasin", "message"=>"Code postal_magasin non renseigné.");}
        if(empty($_POST['adresse_magasin'])) {$messages_erreurs2[sizeof($messages_erreurs2)]=array("field"=>"adresse_magasin", "message"=>"adresse_magasin non renseignée.");}
        if(empty($_POST['telephone_magasin'])) {$messages_erreurs2[sizeof($messages_erreurs2)]=array("field"=>"telephone_magasin", "message"=>"Téléphone non renseigné.");}
        $_POST['telephone_magasin'] = str_replace("+33", "0", $_POST['telephone_magasin']);
        if(!preg_match("#^[0-9]{10}$#", $_POST['telephone_magasin'])) { $messages_erreurs2[sizeof($messages_erreurs2)] = array("field"=>"telephone_magasin", "message"=>"Le numéro de téléphone n'est pas correct, il doit être au format 0601020304."); }
        
        if(!preg_match("#^[a-z0-9_-]{3,15}$#i", $_POST['usershop'])) { $messages_erreurs2[sizeof($messages_erreurs2)] = array("field"=>"usershop", "message"=>"Le nom d'utilisateur ne peut contenir que \"a-z A-Z 0-9 _ -\" et doit faire entre 3 et 15 caractères"); } // Format nom d'utilisateur
        ...
        $getNbUsers = $getNbUsers->rowCount();      
        if($getNbUsers != 0) {
            $messages_erreurs2[sizeof($messages_erreurs2)]=array("field"=>"usershop", "message"=>"Le nom du magasin est déjà utilisé.");
        }

        if (is_int($_POST['postal_magasin'])){$messages_erreurs2[sizeof($messages_erreurs2)]=array("field"=>"postal_magasin", "message"=>"Le code postal_magasin n'est pas au bon format");}

        $_POST['datenaiss'] = str_replace("/", "-", $_POST['datenaiss']);
        if(!preg_match("#^[0-9]{2}-[0-9]{2}-[0-9]{4}$#i", $_POST['datenaiss'])) { $messages_erreurs2[sizeof($messages_erreurs2)] = array("field"=>"datenaiss", "message"=>"La date de naissance n'est pas au format correct !"); } //Format de date invalide

        else {
            $timestamp_naissance = strtotime($_POST['datenaiss']);
            if(time() < $timestamp_naissance) { $messages_erreurs2[sizeof($messages_erreurs2)] = array("field"=>"datenaiss", "message"=>"Mon dieu Marty ! C'est toi !? Tu as retrouvé la DeLorean ?"); } // Date de naissance dans le futur
        }
