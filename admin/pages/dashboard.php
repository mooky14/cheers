...
if($_GET['action'] == "supprimer") {
    echo $comment->id;
   $req = $db->query("DELETE FROM comments WHERE id = ".$_GET['com']);
}


if($_GET['action'] == "valider")
    echo $comment->id;
    $req = $db->query("UPDATE comments SET seen='1' WHERE id=".$_GET['com']);
?>
        ...
        if(!empty($comments)) {
            foreach ($comments as $comment) {
                ...
                <tr id="commentaire_<?= $comment->id ?>">
                    ...
                    <td>
                        <a href="index.php?page=dashboard&action=valider&com=<?php echo $comment->id; ?>" id="<?= $comment->id ?>"
                           class="btn-floating btn-small waves-effect waves-light green"><i
                                class="material-icons">done</i></a>
                        <a href="index.php?page=dashboard&action=supprimer&com=<?php echo $comment->id; ?>" id="<?= $comment->id ?>"
                        ...
                        <div class="modal" id="comment_<?= $comment->id ?>">
                            ...
                            <div class="modal-footer">
                                <a href="index.php?page=dashboard&action=supprimer&com=<?php echo $comment->id; ?>" id="<?= $comment->id ?>"
                                   class="modal-action modal-close waves-effect waves-red btn-flat"><i
                                        class="material-icons">delete</i></a>
                                <a href="index.php?page=dashboard&action=valider&com=<?php echo $comment->id; ?>" id="<?= $comment->id ?>"
